...
const signup = async (req, res, next) => {
  try {
    ...
    await transport.sendMail({
      ...
      to: savedUser.email,
      subject: 'Tabula - Confirm Your Email Address',
    ...
    const maxAge = 100 * 60 * 60 * 24 * 3; // 3 days
...
const login = async (req, res, next) => {
  try {
    ...
    const maxAge = 100 * 60 * 60; // Set cookie in browser to store auth state for 1 hour
    res.cookie('token', token, {
      httpOnly: true,
      maxAge: maxAge,
      domain: process.env.DOMAIN,
  ...
  try {
    ...
    if (user) {
      ...
      await transport.sendMail({
        from: process.env.MAIL_SENDER,
        to: savedUser.email,
        subject: 'Tabula - Password Reset Request',
    ...
    res.status(200).json({
      message:
        'Password reset request successful! If an account exists with the provided email, please check your inbox.',
    });
  } catch (err) {
    next(err);
  }
...
const activateAccount = async (req, res, next) => {
  ...
  try {
    ...
    if (!user) {
      const err = new Error('Invalid or expired activation code');
      err.statusCode = 422;
      throw err;
    }
